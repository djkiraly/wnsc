      ...
      {showConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white rounded-lg shadow-lg p-6 max-w-md w-full mx-4">
            <h3 className="text-lg font-semibold mb-2">Delete Attraction</h3>
            <p className="text-gray-600 mb-6">Are you sure you want to delete &quot;{name}&quot;?</p>
            <div className="flex justify-end gap-3">
              <button onClick={() => setShowConfirm(false)} className="btn btn-outline" disabled={deleting}>Cancel</button>
              <button onClick={handleDelete} className="btn bg-red-600 text-white hover:bg-red-700" disabled={deleting}>{deleting ? 'Deleting...' : 'Delete'}</button>
            </div>
          </div>
        </div>
      )}
