...
export default function DeleteButton({ id, name }: { id: string; name: string }) {
  ...
  const handleDelete = async () => {
    setDeleting(true);
    try { const r = await fetch(`/api/accommodations/${id}`, { method: 'DELETE' }); if (r.ok) router.refresh(); else alert('Failed'); }
      ...
      {showConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white rounded-lg p-6 max-w-md w-full mx-4">
            <h3 className="text-lg font-semibold mb-2">Delete Accommodation</h3>
            <p className="text-gray-600 mb-6">Delete &quot;{name}&quot;?</p>
